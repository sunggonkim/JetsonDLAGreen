...
The public proposed-system name is **QUIET**. Reporting now has three visibly
separate layers:

1. **Common numeric coverage:** every end-to-end table and graph repeats
   **QUIET, NVIDIA MIG, NVIDIA MPS, XSched, Orion, Pantheon** in that order.
   All six consume the same 90 labelled ImageNette inputs, operational
   arrivals, artifacts, and 2,224.448-us deadline. This one-session gate is
   directional evidence, not a formal rank.
2. **Formal promotion:** only QUIET, NVIDIA MPS, and native XSched were enrolled
   in the six-session thermal campaign. MIG, Orion, and Pantheon remain in the
   same table order with an explicit not-enrolled reason.
3. **Partial evidence:** BLESS, GSLICE, gpulet, BOER, ParvaGPU, and DeepPlan
   remain visible in a separate table reporting their positive control or
   measured incompatibility. They are not mixed into the common graph.

The first README table is reason-only for systems without a current end-to-end
row. The fixed common roster, formal ledger, and accuracy table follow it.
`analysis/generate_p9_six_system_figure.py` regenerates the common graph;
`analysis/generate_p9_current_figures.py` regenerates the paper tables. In
`docs/p9-comparator-manifest.json`, `fixed_numeric_roster` and
`executed_result_order` freeze the common order, `partial_evidence_order`
freezes the separate partial table, and `direct_ranking_order` identifies the
formal subset. The older `numeric_frontier_order` remains only for exact-lock
analysis scripts.
