...
\subsection{Native Comparator Fidelity}

\PnineComparatorTable

Table~\ref{tab:native-comparators} distinguishes fidelity from rankability.
XSched's native path participates in the complete formal campaign and is
infeasible at the frozen deadline.  Pantheon executes its pinned online
scheduler, priority tiers, and model chunks on the same 90-request ImageNette
gate.  It preserves 0.8333 accuracy, records two misses, and has a
$4{,}133~\mu$s p99.  Its protobuf interface floors a distinct
$2{,}224.448~\mu$s deadline to integer microseconds; we therefore count it as
the second faithful published mechanism but do not pool or rank it against
Table~\ref{tab:formal-campaign}.

Orion remains excluded because its upstream differential scheduler trace is
not complete.  The local port's ability to launch the workload does not repair
that missing mechanism evidence.  gpulet's native planner cannot form a
feasible common dependent plan, and independent-service systems are retained
as structural controls.  These negative boundaries are part of the result:
executable equivalence, common inputs, and comparable statistics are separate
requirements.
...
marked, and Pantheon remains a separate fidelity gate.
