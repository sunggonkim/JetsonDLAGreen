...
\begin{figure*}[t]
  ...
  rank; MIG uses its valid BE-capable layout, with the critical DAG on 2g and
  the background tenant on the isolated 1g instance.}
...
with p99s of $1{,}933.446$, $1{,}514.253$, and
$1{,}749.850~\mu$s.  MIG co-locates producer and consumer on 2g and isolates
BE on 1g, completing 249.319 background requests/s.  Thus MIG remains a full
six-system comparator for the observed DMR--goodput objective rather than a
no-BE capacity endpoint.
...
microseconds.  Five systems preserve 0.8333 accuracy; MIG records 0.8444 versus
the 0.8333 reference and passes its exploratory semantic gate.  With only 90
...
\begin{figure*}[t]
  ...
  stages on 2g and admits BE on the isolated 1g instance.  The latter is the
  primary six-system MIG row; the former is a no-BE capacity control.}
...
Figure~\ref{fig:mig-topology-partial} explain the primary MIG topology.  Thor
exposes at most two simultaneous instances; a 1g producer, 1g
consumer, and third 1g background partition is not a supported placement.  We
therefore use the valid alternative as the six-system MIG row: producer and
consumer share 2g while BE runs on 1g.  It replays the same 90 inputs, arrival trace, and
...
configured 0.02 exploratory tolerance.  The split 1g+2g layout remains visible
as a secondary capacity control; it records a $1{,}652.861~\mu$s p99 but cannot
admit BE.  Neither one-session topology supports a formal repeated-session
ranking claim.
...
primary six-system MIG result is a directional 2g-DAG+1g-BE measurement, not
part of that formal campaign; Thor provides no valid three-instance layout.  The
