...
and is not pooled with the six-session table.  Orion~\cite{orion2024} is
reported as separately scoped numeric evidence because its upstream
differential decision gate is incomplete.  BLESS~\cite{bless2025} and
independent-service provisioners are reported as executed mechanism
boundaries when their published mechanisms cannot execute the identical
dependency contract.
...
Our evaluation produces five results.  First, both a 90-request ImageNette
...
a three-slot nonthermal Whisper motivation identifies the regime that gives
\sys purpose beyond MIG isolation: at 19 requests/s, MIG misses 167/300
deadlines, static MPS misses 64/300, and \sys misses 0/300 while preserving
byte-identical outputs.  Third, same-activation causal replay shows that
precedence changes shared-resource overlap, not merely copy time: across three
exploratory pairs, \sys's
dependent execution has a $2{,}081.286~\mu$s lower mean p99 than the replayed
independent arm.  Fourth, in the thermal-normalized campaign, \sys is the only
tested system whose 95\% upper deadline-miss bound qualifies for the 0.05\%
target.  Fifth, offered-load points reveal the expected statistical boundary:
...
\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
  ...
  a bounded three-slot real-ASR path, request-indexed activation replay, and
