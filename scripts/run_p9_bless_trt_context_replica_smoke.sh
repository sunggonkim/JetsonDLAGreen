...
result_dir=${RESULT_DIR:-"$repo/results/p9-bless-trt-context-replica-$(date -u +%Y%m%dT%H%M%SZ)"}
...
env \
  ...
  taskset --cpu-list 13 "$repo/build-r39/bless-trt-context-replica-smoke" \
  --replica-rounds 2 --engine "$engine" --model-name distilbert-sst2 \
  --role benchmark --samples 20 --warmup 5 --include-transfers true \
  --priority default >"$result_dir/result.json" 2>"$result_dir/stderr.txt"

cp "$repo/build-r39/bless-trt-context-replica-smoke" "$result_dir/provenance/"
cp "$capture" "$result_dir/provenance/"
cp "$repo/baselines/orion/driver_capture/intercept_driver.cpp" "$result_dir/provenance/"
cp "$repo/baselines/bless/trt_context_replica_smoke.cpp" "$result_dir/provenance/"
cp "$repo/baselines/bless/verify_trt_context_replica_smoke.py" "$result_dir/provenance/"
cp "$repo/scripts/run_p9_bless_trt_context_replica_smoke.sh" "$result_dir/provenance/"
python3 "$repo/baselines/bless/verify_trt_context_replica_smoke.py" \
